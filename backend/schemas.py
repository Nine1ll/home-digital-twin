...
class Signup(BaseModel):
    ...
    def password_bytes(cls, v):
        if len(v.encode()) > 72:
            raise ValueError("비밀번호는 UTF-8 72바이트 이하로 입력하세요")
        return v
...
class LocationInput(BaseModel):
    ...
    width: float = Field(default=4, ge=0.5, le=20, allow_inf_nan=False)
    height: float = Field(default=3, ge=0.5, le=20, allow_inf_nan=False)
...
class ItemInput(BaseModel):
    ...
    def not_blank(cls, v):
        if not v.strip():
            raise ValueError("이름과 단위를 입력하세요")
        return v.strip()
